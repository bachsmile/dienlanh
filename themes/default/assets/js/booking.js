!(function (o) {
	...
	(o.modal.prototype = {
		...
		show: function () {
			...
			((this.closeButton = o('<a href="#close-modal" rel="modal:close" class="close-modal ' + this.options.closeClass + '">' + this.options.closeText + "</a>")), this.$elm.append(this.closeButton)),
		...
		showSpinner: function () {
			...
			((this.spinner = this.spinner || o('<div class="' + this.options.modalClass + '-spinner"></div>').append(this.options.spinnerHtml)), this.$body.append(this.spinner), this.spinner.show());
		},
...
var booking = {
	...
	doSubmit: function(f){
		booking.loadAjax({
			url: ROOT_MOD + '/ajax/sendForm.html',
	...
	elmCheck: function (e) {
		return $("body, html").find(e).length;
	},
	getValue: function (e, i) {
		var a = [];
		return (
			e.find("li").each(function (e, i) {
				var n = $(i).data("service");
				$(i).hasClass("active") && a.push(n);
			}),
			i.val(a.join()),
			a.join()
		);
	},
	form_booking_online: function () {
		var e = $("#booking-online"),
		i = $(".box-form");
		$(".package_service")
		.find("li")
		.each(function (e, i) {
			$(i).on("click", function () {
				$(this).hasClass("active") ? $(this).removeClass("active") : $(this).addClass("active"), booking.getValue($(".package_service"), $('input[name="package_service"]'));
			});
		}),
		i.submit(function (a) {
			a.preventDefault(a);
			var n = i.find('input[name="package_service"]').val();
			let t = !0;
			0 == n.length && (alert("Vui lĂ²ng chá»n gĂ³i dá»‹ch vá»¥ báº¡n cáº§n."), (t = !1)),
			t &&
			$.ajax({
				async: !0,
				url: ajaxurl+'?doaction=booking',
				type: "POST",
				dataType: "json",
				data: $(".box-form").serialize(),
				beforeSend: function () {
					e.addClass("ajax-processing"), e.append('<div class="lds-ellipsis"><span></span><span></span><span></span><span></span></div>'), i.find("button").addClass("processing");
				},
				success: function (response) {
					e.removeClass("ajax-processing"),
					e.find(".lds-ellipsis").remove(),
					i.find("button").removeClass("processing"),
	...
	init: function(){
		...
		booking.form_booking_online();
		$("#time").bootstrapMaterialDatePicker({ date: !1, format: "HH:mm" });
		$("#date").bootstrapMaterialDatePicker({ weekStart: 0, time: !1, format: "DD/MM/YYYY", lang: "vi", minDate: new Date() });
	},
};
